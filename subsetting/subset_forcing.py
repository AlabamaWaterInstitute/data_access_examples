...
def main():
    ...
    parser.add_argument(dest="forcing_dir_out", type=str, help="Path to output the forcing files subset")
    parser.add_argument(dest="catchment_file",  type=str, help="A catchment geojson file")    
        ...
        for jfeat in feats:
            ...
            for jforcing in forcing_files:
                if jforcing.find(cat_id) >= 0:
                    ...
                    os.system(f'cp {os.path.join(indir,jforcing)} {os.path.join(outdir,jforcing)}')
            if not found: 
