...
def setup_arguments():
    ...
    parser.add_argument('input_files',
                        ...
                        type=str,
                        nargs='+',
                        help='Input files, with the first entry used as a '
                             'template')
    ...
    parser.add_argument('-t',
                        ...
                        type=str,
                        default='time',
                        help='Specify the name of the time '
                             'variable, otherwise defaults to "time"',
                        metavar='VARIABLE')
    ...
    parser.add_argument('-c',
                        ...
                        type=int,
                        default=5120,
                        help='Specify the number of rows within a chunk'
                             '. Defaults to 5120.',
                        metavar='ROWS')
    # Max memory in bytes
    ...
    parser.add_argument('-s',
                        ...
                        action='store',
                        type=str,
                        help='Route logging messages to syslog facility '
                             'FACILITY',
                        metavar='FACILITY')
...
def validate_common_dimensions(input_files, skip_missing):
                ...
                with Dataset(filename, 'r') as input_file:
                    ...
                    for dimension_name in archetype_list.keys():
                        ...
                        if ((dimension_name not in other_dimensions) or
                            ...
                            raise ValueError('The NetCDF file {0} does not '
    ...
    if len(input_files) != len(new_input_files):
        logger.info('Updating input file list with %d subtraction(s)',
            ...
            with Dataset(filename, 'r') as input_file:
                ...
                for variable_name in variable_names:
                    if variable_name not in input_file.variables:
                        raise ValueError('Variable {0} not present in '
...
#
##########################################################################
def copy_attributes(input_variable, output_variable,
    ...
    for key in input_variable.ncattrs():
        ...
        if (not removed_attributes or
                key not in removed_attributes):
...
def copy_coordinate_variables(input_file, output_file, excluded_variables):
    ...
    for variable_name in coordinate_variables:
        ...
        output_coordinate = output_file.createVariable(
            variable_name, coordinate.dtype, (variable_name,),
...
#
#######################################################################
def create_processing_chunks(number_files, number_rows, chunk_rows,
                             max_memory):
    ...
    # if possible.
    batch_rows = min(int(math.ceil(min(max_memory, float(number_rows) *
                                       number_files * bytes_per_value) /
                                   (bytes_per_value * number_files) /
                                   chunk_rows)) * chunk_rows, number_rows)

    # Have to make sure is not one single row (range does not like step 0)
    if batch_rows != 0:
        for row in range(batch_rows, number_rows - 1, batch_rows):
            processing_chunks.append([previous_row,
                                      previous_row + batch_rows])
            previous_row = row
...
#
############################################################################
def create_time_series_variable(input_file, output_file, variable_name,
                                time_variable, reference_time_variable,
                                input_filepaths, chunk_rows, max_memory):
    ...
    output_variable = output_file.createVariable(variable_name,
                                                 variable.dtype,
    ...
    processing_chunks = create_processing_chunks(len(input_filepaths),
                                                 len(input_file.dimensions[variable.dimensions[0]]),
                                                 chunk_rows, max_memory)
    ...
    if variable_name in [time_variable, reference_time_variable]:
        ...
        for input_data_file in input_filepaths:
            with Dataset(input_data_file, 'r') as data_frame:
                ...
                # rounding issues
                variable_arrays.append(date2num(num2date(data_frame.variables[variable_name][:],
        ...
        logger.info('Restacking %s using %d processing chunks',
                    variable_name, len(processing_chunks))
            ...
            for input_data_file in input_filepaths:
                ...
                with Dataset(input_data_file, 'r') as data_frame:
                    ...
                    variable_arrays.append(
                        data_frame.variables[variable_name][chunk_range[0]:chunk_range[1]])

            output_variable[chunk_range[0]:chunk_range[1], :] = \
                numpy.column_stack(variable_arrays)
...
#
############################################################################
def create_time_series(input_filepaths, output_filepath, data_variables,
                       time_variable, reference_time_variable, chunk_length,
                       max_memory, removed_global_attributes, skip_missing):
    ...
    try:
        ...
        input_filepaths = validate_variables(input_filepaths, data_variables +
                                             [time_variable,
                                              reference_time_variable],
                                             skip_missing)
        input_filepaths = validate_common_dimensions(input_filepaths,
                                                     skip_missing)

        # Open output file
        with Dataset(output_filepath, 'w', clobber=True,
                     format='NETCDF4_CLASSIC') as output_file:
            remove_output = True
            ...
            with Dataset(input_filepaths[0], 'r') as archetype:
                ...
                copy_dimensions(archetype, output_file,
                                [time_variable, reference_time_variable],
                                len(input_filepaths))

                # Copy coordinate variables from the archetype
                # (excluding time-related ones)
                copy_coordinate_variables(archetype, output_file,
                                          [time_variable,
                                           reference_time_variable])
                ...
                for variable in data_variables:
                    ...
                    create_time_series_variable(archetype, output_file,
                                                variable, time_variable,
                                                reference_time_variable,
                                                input_filepaths, chunk_length,
                                                max_memory)
...
def main():
    ...
    try:
        ...
        # Do work
        create_time_series(options.input_files, options.output_file,
